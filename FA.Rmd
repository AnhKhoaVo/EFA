...
```{r, echo=FALSE}
fa <- fa(cfa1_week0[-1:-3], nfactors=6, rotate="varimax", fm="ml")
fa.diagram(fa)
```
...
model0 <- 'Factor 1 =~ HGB00 + HCT00 + RBC00 
           ...
           TLAMS52 ~ Factor 1 + Factor 2 + Factor 3 + Factor 4 + Factor 5 + CHC00'

f1_week0 <- sem(model0, data = cfa1_week0, std.lv=TRUE, na.action=na.omit, missing = "fiml")
...
factor_lm <- function(x) {
  f1 <- summary(lm(TLAMS52 ~ Factor1, data = x))
  f2 <- summary(lm(TLAMS52 ~ Factor2, data = x))
  f3 <- summary(lm(TLAMS52 ~ Factor3, data = x))
  f4 <- summary(lm(TLAMS52 ~ Factor4, data = x))
  f5 <- summary(lm(TLAMS52 ~ Factor5, data = x))
  f6 <- summary(lm(TLAMS52 ~ CHC00, data=x))
...
```{r, echo=FALSE}
clusters <- c("Factor1", "Factor2", "TLAMS52", "RBC00", "HGB00", "HCT00", "STP00", "ALB00", "CAB00", "CHO00", "CHC00")
factors <- subset(cfa2_week0, select=(clusters))
factors <- as.data.frame(cor(factors, use="pairwise.complete.obs"))
Values <- as.data.frame(factors[1:6])
Values
